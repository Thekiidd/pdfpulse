// src/components/tools/PageNumbersTool.jsx
import { useState } from 'react';
import { motion } from 'framer-motion';
import { DocumentIcon, HashtagIcon } from '@heroicons/react/24/outline';
import { PDFDocument, StandardFonts } from 'pdf-lib';

export default function PageNumbersTool({ incrementCounter }) {
  ...
  const addPageNumbers = async () => {
    ...
    try {
      ...
      pages.forEach((page, i) => {
        ...
        switch (position) {
          case 'top-left':
            ...
            break;
          case 'top-right':
            x = width - textWidth - 50; y = height - 30;
            break;
          case 'bottom-left':
            x = 50; y = 30;
            break;
          case 'bottom-right':
          default:
            x = width - textWidth - 50; y = 30;
            break;
        }

        page.drawText(text, {
          x, y, size: fontSize, font, color: { type: 'RGB', red: 0, green: 0, blue: 0 }
        });
      });
      ...
      a.href = url;
      a.download = `pdfpulse_pagenumbers_${now}.pdf`;
      a.click();
    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-8">
      ...
      >
        ...
        {file ? (
          ...
          <div className="space-y-4">
            ...
            >
              <HashtagIcon className="w-10 h-10 text-neon" />
            </motion.div>
      ...
      {loading && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-2">
          ...
          <p className="text-center text-sm text-gray-400">Procesando páginas...</p>
        </motion.div>
      )}
    </motion.div>
  );
