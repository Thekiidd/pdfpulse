      ...
      <div className="relative z-10 min-h-screen flex flex-col">
        ...
        <main className="flex-grow max-w-5xl mx-auto px-4 py-24">
          ...
          <div className="relative">
            ...
            {/* --- CONTENIDO FALSO (detrás del overlay) --- */}
            {/* Usamos 'blur-md' para desenfocar y 'pointer-events-none' 
            ...
            */}
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 blur-md pointer-events-none select-none">
              ...
              <TokenPackage name="Paquete Business" tokens={1000} price="35" />
            </div>

            {/* --- EL OVERLAY "EN CONSTRUCCIÓN" --- */}
            <div className="absolute inset-0 flex flex-col items-center justify-center 
                            bg-black/50 backdrop-blur-sm rounded-3xl border border-neon/30 text-center p-8">
              ...
              <Link
                to="/dashboard"
                className="btn-neon px-6 py-3 text-sm font-bold tracking-wider rounded-lg"
              >
                Volver al Dashboard
              </Link>
            </div>
...
function TokenPackage({ name, tokens, price, popular = false }) {
  return (
    <div className={`p-8 rounded-3xl border ${popular ? 'border-neon' : 'border-neon/30'} bg-white/5`}>
      {popular && <span className="text-xs font-bold text-neon bg-neon/10 px-3 py-1 rounded-full">POPULAR</span>}
