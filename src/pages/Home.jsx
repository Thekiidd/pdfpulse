...
export default function Home() {
    ...
    const tools = [
        ...
        { id: 'ai-pdf', name: "Chat con PDF (IA)", desc: "Pregunta a tu documento", icon: SparklesIcon, component: <AiPdfTool />, fullScreen: true },
...
function ToolView({ tool, onBack, incrementCounter, user, userData }) {
    ...
    return (
        <>
            ...
            <main className={isFullScreen ? "w-full h-[calc(100vh-80px)] mt-20 px-4" : "max-w-4xl mx-auto px-4 py-8 mt-16"}>
                <motion.div
                    initial={{ opacity: 0, scale: 0.95 }}
                    animate={{ opacity: 1, scale: 1 }}
                    transition={{ duration: 0.4 }}
                    className={isFullScreen
                        ? "w-full h-full bg-black/80 backdrop-blur-xl border border-neon/30 shadow-neon-lg rounded-2xl overflow-hidden"
                        : "bg-white/5 backdrop-blur-xl p-8 border border-neon/30 shadow-neon-lg"
                    }
                >
                    {!isFullScreen && (
